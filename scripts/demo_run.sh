...
const verdict = scanPayload.verdict || {};
const capAssessment = verdict.capabilities_assessment || {};

console.log("");
console.log(`scan_id: ${scanPayload.scan_id || "unknown"}`);
console.log(`verdict: ${verdict.result || "UNKNOWN"}`);
console.log(`method: ${verdict.method || "unknown"}`);
console.log(`risk_score: ${verdict.risk_score ?? "?"}`);
console.log(`confidence: ${verdict.confidence ?? "?"}`);

const keys = Object.keys(capAssessment);
if (keys.length) {
  console.log("");
  console.log("capabilities_assessment:");
  for (const key of keys.slice(0, 8)) {
    console.log(`- ${key}: ${String(capAssessment[key]).slice(0, 160)}`);
  }
}
